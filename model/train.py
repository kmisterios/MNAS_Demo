...
from model.model import BERT_for_AS_cossim


def train_eval(model_name, data, batch_size, batch_size_val, linear_layer_size, num_epochs, result_dict,
               train = True, checkpoints_path = "./checkpoints", figs_path = "./figs", inference = False):
    ...
    embedder = BERT_for_AS_cossim(model, linear_layer_size)
    embedder.to(device)
    embedder.load_state_dict(torch.load(f'{checkpoints_path}/model_best_{model_name}_{num_epochs}.pth', map_location=torch.device('cpu')))
    criterion = MSELoss()
    if not inference:
        ...
        x1 = list(np.array(list(data['content'].values))[:, 0])
        x2 = list(np.array(list(data['content'].values))[:, 1])
        tokens_info1 = tokenizer(x1, padding=True, truncation=True, return_tensors="pt")
        tokens_info2 = tokenizer(x2, padding=True, truncation=True, return_tensors="pt")
        valid_data = TensorDataset(tokens_info1['input_ids'], tokens_info1['attention_mask'],tokens_info2['input_ids'], tokens_info2['attention_mask'])
        ...
        for batch in valid_dataloader:
            batch = tuple(t.to(device) for t in batch)
            input_ids1, attention_mask1, input_ids2, attention_mask2 = batch
